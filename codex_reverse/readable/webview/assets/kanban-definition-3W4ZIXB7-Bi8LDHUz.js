...
import { g as Ne } from "./chunk-FMBD7UC4-w2inTx7J.js";
var $ = (function () {
...
` + e,
    ...
    const I = l.append("g");
    I.attr("class", "items");
    const g = _.nodes.filter((m) => m.isGroup);
    let w = 0;
    const E = 10,
      G = [];
    let N = 25;
    ...
    for (const m of g) {
      ...
      const L = b?.kanban?.sectionWidth || 200,
        M = (-L * 3) / 2 + N;
      let T = M;
      const Y = _.nodes.filter((i) => i.parentId === m.id);
      for (const i of Y) {
        if (i.isGroup)
          throw new Error(
...
`,
  ...
  Me = Ge,
  We = { db: Ve, renderer: je, parser: xe, styles: Me };
export { We as diagram };
//# sourceMappingURL=kanban-definition-3W4ZIXB7-Bi8LDHUz.js.map
